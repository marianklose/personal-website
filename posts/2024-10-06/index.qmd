...
When I started my PhD in pharmacometrics, I wanted to try something fancy[^1]: specifying a combined proportional and additive error model in NONMEM for one of my projects. A colleague kindly sent me a reference model, and to my confusion, the code included a (at least to me) novel way of defining residual unexplained variability (RUV):
...
Now the *alternative* way (my colleague called it the *Uppsala way*[^3]) of coding the additive error model looks like this:
...
But we then multiply this random variable ```EPS(1)```  by a scaling factor ```SD_ADD``` (which is being estimated as a ```THETA``` parameter) before the product is being added to the individual predicted ```IPRED``` value. Let’s visualize what happens when we fix ```$SIGMA``` to 1 and multiply it by ```SD = 0.23```. 

In a first step we will plot a standard normal distribution (```$SIGMA 1 FIX```):
...
The resulting standard deviation should be 1, and since $1^2 = 1$, the resulting variance should also be 1. Let's check our empirical estimates (it is a simulation, after all) to confirm this:
...
the `SD_ADD` parameter (specified via ```$THETA```) is representing a standard deviation. Cool thing! This is why both of these models are equivalent:
...
To sum it up: We need to be careful with the units. If we use the *classical* way, the ```$SIGMA``` estimate is a variance, but if we use the *alternative* way, the ```$THETA``` estimate is a standard deviation. Typically, we would report the standard deviation (rather than the variance) if we use an additive model, and I think one of the advantages of the *alternative* way is that we directly read out the standard deviation from the parameter estimates (without the need to transform anything). Some also say that the estimation becomes more stable if we model the stochastic parts via ```$THETA```, but I cannot judge if this is true or just a myth.
...
This estimate of 0.15 already serves as a CV, as we are then multiply this by the prediction (```IPRED```) of 10 mg/L to get the standard deviation of the prediction distribution:

$$
\sigma_{PRED} = 0.15 \times 10 = 1.5 \, \text{mg/L}
$$

In the *alternative* way, we are directly given the coefficient of variation (CV) as 0.15. The CV represents the relative variability, so to get the standard deviation, we multiply the CV by the prediction (```IPRED```):

$$
\sigma_{PRED} = 0.15 \times 10 = 1.5 \, \text{mg/L}
$$

In both cases, the resulting variability is the same, meaning both approaches lead to the same standard deviation of 1.5 mg/L. 
...
This is a somewhat lengthy explanation of why we code the *alternative* approach in NONMEM. Personally, I wasn't very familiar with how distributions behave when multiplying by a random variable, and I didn’t realize that while variances are additive when combining two random processes, standard deviations are not. If you have a stronger background in statistics, this might have been obvious, but I hope this explanation was still helpful for some.
