...
Please note: These are just my personal reflections and interpretations of Bayesian concepts. While I've aimed for accuracy, I cannot guarantee that my explanations, equations, or terminology are flawless. If you notice any inaccuracies, please let me know so I can correct them! 
...
![A traffic jam on your way to work. Credits: Aayush Srivastava, Pexels.](media/pexels-aayushsri-1445653.jpg){width=100% #fig-traffic-jam}

Would you stick to your initial belief that it will take you 30 minutes? Probably not, you would likely revise it (or try to break all speed limits). But would you simply double the 25 minutes to predict a 50-minute commute? This is also unlikely, given your experience suggesting that 40 minutes is usually the upper limit. So, maybe you would adjust your estimate to 38 minutes, assuming that the traffic will get better soon. This is Bayesian thinking in action: Updating your beliefs with new data. So far just without any equation and rather based on gut-feeling.
...
# Pharmacometric example and reference solution
...
We will focus on ID 5, which has lower simulated concentrations, making it easier to observe differences between typical and individualized profiles. Here's a plot to visualize these concentration-time profiles of ID 5 and the other IDs of the virtual population:

```{r}
#| label: fig-sim-vis
#| fig-cap: "Simulated concentration-time profiles for 10 individuals after i.v. bolus injection. ID 5 (blue) represents our examplaratory ID."
...
sim_data |> 
  ...
  ggtitle("Simulated concentration time data after i.v. infusion")
```
...
For our example and reference, we'll use a simple one-compartment IV model with first-order kinetics, previously fitted to similar data in @klose2025. Here's the model structure:
...
$$CL_i = 0.247 \cdot \exp(0.55194) = 0.3977$$ {#eq-cl-individual-num}
...
- $p(\eta_i|Y_{ij})$: posterior distribution of parameter $\eta_i$ given the individual data $Y_{i}$
...
Dealing with products of probabilities can be cumbersome (and mathematically instable), so taking the logarithm simplifies the calculation:
...
Now, we can compute the prior term for any given individual $\eta_i$, using variance $\omega^2_{CL}$ (e.g., 0.11) from our NLME model. If we have a good reason to believe that our prior should be different (e.g., we apply our model to another population), we could adjust the variance accordingly.
...
Please note that in our case (a combined additive and proportional RUV model), $\sigma^2$ represents the combined variances at time $t_{ij}$ and prediction $f(x_{ij}$. As $f(x_{ij}$ is a constant for a given $t_{ij}$, we can compute the combined variance as 

$$\sigma^2 = \sigma^2_{prop} \cdot f(x_{ij})^2 + \sigma^2_{add}$$ {#eq-sigma2}

Therefore, we have to calculate the $\sigma^2$ for each data point $Y_{ij}$ individually.

## Combining both terms

Combining the prior and likelihood terms, we obtain:

$$\eta_i^* = \underset{\eta_i}{\mathrm{argmax}}~\left(\left[-0.5 \log(2\pi\omega^2) - \frac{\eta_i^2}{2\omega^2}\right] ~ + \left[-0.5 \log(2\pi\sigma^2) - \frac{(Y_{ij}-f(x_{ij}; \eta_i))^2}{2\sigma^2}\right]\right)$$ {#eq-map-final}
...
Next, we construct the likelihood function for individual observations:
...
Now, we can visualize the likelihood term across a range of $\eta_i$ values:
...
## Comparison of prior, likelihood, and posterior
...
comp_tibble |>  
  ...
  geom_vline(
    ...
    aes(xintercept = map_est$par |> max()),
...
This clearly demonstrates how the posterior is influenced by both the prior and the likelihood, with the MAP estimate reflecting a balance between these two sources of information.


# Conclusion

This simple example illustrates how to estimate the individual parameter $\eta_i$ (which translates to an individual clearance $CL_i$) for a single subject using a Bayesian approach. We have seen that the posterior distribution, often summarized by its mode (the MAP or EBE estimate), represents a compromise between the prior and the likelihood. This powerful technique enables the continuous integration of prior knowledge with new data.
