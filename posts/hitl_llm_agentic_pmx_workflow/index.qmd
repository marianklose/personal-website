...
Lets first dig into some fundamental understanding of LLM-agents, multi-agentic workflows and the specifics of pharmacometrics.
...
1) a specific model, e.g., openai::gpt-5.5 or claude::sonnet-4.6, or a locally ran LLM model like xxx. This defines the capabilities of the model, the cost, the speed, and maybe where this model is particularly good at
2) a set of system messages/instructions that define the tone and the 
3) a set of tools that the agent is able to call, e.g. a web search or a personally defined python function
...
4) We define a structured output format so that it gives back a 

## What is a multi-agentic workflow?

The current development seems to rather go into the direction of having many single LLM-agents that are specialized (through model, system message tailored to the task, tools, structured output format) in a specific task than having one single agent that is capable of doing everything. That is how multi-agentic workflow evolved: A complex task is prompted to a orechestrator/supervisor agent, which is then broken down to many subtasks that are handed to the specialized agents, and in the end the orchestrator/supervisor agent receives the output from all of them to fulfull the task. 


## What are the special challenges in pharmacometrics?

- Vibe coding is nice, sure. And it is impressive how quickly you can generate code with simple natural language. But in pharmacometrics we cannot simply let a multi-agent workflow perform an analysis without having any human in the loop. In the end we care about the output, the predicted AUC, Cmax, risk, or whatever numeric we are interested in. If we do not fully trust these numbers, than our analysis is useless. The decisions based on these numbers are simply to important to hand it all over to an agent.
